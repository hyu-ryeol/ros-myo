...
class MyoRaw(object):
    ...
    def connect(self):
        ...
        def handle_data(p):
            ...
            if attr == 0x27:
                ...
                self.on_emg(emg, moving, rms_data)
            elif attr == 0x1c:
                vals = unpack('10h', pay)
                quat = vals[:4]
                acc = vals[4:7]
    ...
    def on_emg(self, emg, moving, rms_data):
        for h in self.emg_handlers:
            h(emg, moving,rms_data)
...
if __name__ == '__main__':
    ...
    def proc_emg(emg, moving, rms_data):
        # create an array of ints for emg data
        # and moving data
        emg_data = np.delete(emg,(3),axis=0)
        rms_data = np.power(emg_data, 2)

        window = np.ones(250)/float(250)
        # print(window)
        rms_data = np.sqrt(np.convolve(rms_data, window, 'valid'))
        rms_data = np.mean(rms_data)

        #nor_data = (rms_data- np.min(rms_data)) / (np.max(rms_data) - np.min(rms_data))

        map_value = (rms_data - emg_min) * (gain_max - gain_min) / (emg_max - emg_min) + gain_min

        # if map_value == 0:
        #     map_value = gain_min

        if map_value > gain_max :
            map_value = gain_max
        if map_value < 0:
            map_value = gain_min

        # map_value=round(map_value,0)
        print(map_value)
        emgPub.publish(EmgArray(emg, moving,int(map_value)))
